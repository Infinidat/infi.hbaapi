...
def get_port_object(adapter_attributes=Munch(), port_attributes=Munch()):
    ...
    kwargs['port_symbolic_name'] = getattr(port_attributes, "PortSymbolicName").split('\x00', 1)[0]
    kwargs['os_device_name'] = getattr(port_attributes, "OSDeviceName").split('\x00', 1)[0]
    ...
    kwargs['driver_name'] = getattr(adapter_attributes, "DriverName", '').split('\x00', 1)[0]
    kwargs['driver_version'] = getattr(adapter_attributes, "DriverVersion", '').split('\x00', 1)[0]
    kwargs['manufacturer'] = getattr(adapter_attributes, "Manufacturer", '').split('\x00', 1)[0]
    kwargs['serial_number'] = getattr(adapter_attributes, "SerialNumber", '').split('\x00', 1)[0]
    kwargs['model'] = getattr(adapter_attributes, "Model", '').split('\x00', 1)[0]
    kwargs['model_description'] = getattr(adapter_attributes, "ModelDescription", '').split('\x00', 1)[0]
    kwargs['hardware_version'] = getattr(adapter_attributes, "HardwareVersion", '').split('\x00', 1)[0]
    kwargs['firmware_version'] = getattr(adapter_attributes, "FirmwareVersion", '').split('\x00', 1)[0]
    kwargs['option_rom_version'] = getattr(adapter_attributes, "OptionROMVersion", '').split('\x00', 1)[0]
    port = Port()
